...
uint8_t Mscale =
    MFS_16BITS;  // Choose either 14-bit or 16-bit magnetometer resolution
uint8_t Mmode =
    0x02;  // 2 for 8 Hz, 6 for 100 Hz continuous magnetometer data read
...
float magCalibration[3] = {0, 0, 0}, magbias[3] = {0, 0, 0},
      magscale[3] = {0, 0, 0};  // Factory mag calibration and mag bias
float gyroBias[3] = {0, 0, 0},
      accelBias[3] = {0, 0, 0};  // Bias corrections for gyro and accelerometer
int16_t tempCount;               // temperature raw count output
float temperature;  // Stores the real internal chip temperature in degrees
                    // Celsius
...
void setup() {
  ...
  if (c == 0x71 || c == 0x73 || c == 0xFF)  // WHO_AM_I should always be 0x68
  {
    ...
    calibrateMPU9250(gyroBias, accelBias);  // Calibrate gyro and
                                            // accelerometers, load biases in
                                            // bias registers
    delay(1000);
    ...
    byte d = readByte(AK8963_ADDRESS,
                      AK8963_WHO_AM_I);  // Read WHO_AM_I register for AK8963
    Serial.print("AK8963 ");
    Serial.print("I AM ");
    Serial.print(d, HEX);
    Serial.print(" I should be ");
  ...
  */
  while (Serial.available()) {
    ...
    if (in == 'c') {
      // force to reset
      // reset();
      Serial.println("IMU: Arduino got reset signal from host");
      angle = 0;
      start_angle = 0;
      orig_angle = 0;
      old_time = 0;
  ...
  if (readByte(MPU9250_ADDRESS, INT_STATUS) &
      0x01) {                   // On interrupt, check if data ready interrupt
    readAccelData(accelCount);  // Read the x/y/z adc values
    getAres();

    // Now we'll calculate the accleration value into actual g's
    ax = (float)accelCount[0] * aRes;  // - accelBias[0];  // get actual g
    ...
    gx = (float)gyroCount[0] *
         gRes;  // get actual gyro value, this depends on scale being set
    ...
    getMres();
#if NEEDED
    magbias[0] = +470.;  // User environmental x-axis correction in milliGauss,
  ...
  // We have to make some allowance for this orientationmismatch in feeding the
  ...
  if (!AHRS) {
    ...
    if (delt_t > 500) {
      if (SerialDebug) {
        ...
        temperature = ((float)tempCount) / 333.87 +
                      21.0;  // Temperature in degrees Centigrade
    ...
    // if (delt_t > 500) { // update LCD once per half-second independent of
    // read rate
    if (delt_t > 62) {  // update serial output every 62ms
      ...
      // Yaw is the angle between Sensor x-axis and Earth magnetic North (or
      // true North if corrected for local declination, looking down on the
      // sensor positive yaw is counterclockwise.
      ...
      // the get the correct orientation the rotations must be
      // applied in the correct order which for this configuration is yaw,
      // pitch, and then roll.
      // For more see
      ...
      pitch *= 180.0f / PI;
      yaw *= 180.0f / PI;
      yaw -= 13.8;  // Declination at Danville, California is 13 degrees 48
      ...
      if (true) {
...
#endif
        if (count != 0) {
          // gyro dift compensation: we will do it in cart_status.cpp
          orig_angle = angle;
        ...
        Serial.println("<qW:" + String(angle, 4) + ",qX:" +
                       String(orig_angle, 4) + ",qY:" + String(start_angle, 4) +
                       ",qZ:" + String(0.0, 4) + ",eX:" + String(yaw, 4) +
                       ",eY:" + String(pitch, 4) + ",eZ:" + String(roll, 4) +
                       ">\r\n");
...
void readMagData(int16_t* destination) {
  ...
  if (readByte(AK8963_ADDRESS, AK8963_ST1) &
    ...
    if (!(c & 0x08)) {  // Check if magnetic sensor overflow set, if not then
      ...
      destination[1] = ((int16_t)rawData[3] << 8) |
...
void initMPU9250() {
  ...
  writeByte(MPU9250_ADDRESS, SMPLRT_DIV, 0x04);  // Use a 200 Hz rate; a rate
...
void MPU9250SelfTest(float* destination)  // Should return percent deviation
                                          // from factory trim values, +/- 14 or
